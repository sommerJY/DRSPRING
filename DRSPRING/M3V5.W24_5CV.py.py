...
SAVE_PATH = '/home01/k020a01/02.M3V5/M3V5_W24_349_DATA/'
# SAVE_PATH = '/st06/jiyeonH/11.TOX/DR_SPRING/trials/M3V5_W24_349_FULL/'

file_name = 'M3V5_W24_349'
...
WORK_NAME = 'WORK_24' # 349 # only basal 
...
# 테스트 버전 
# data_ind = list(A_B_C_S_SET.index)[0:2048]
...
# 테스트 버전 
# A_B_C_S_SET = A_B_C_S_SET.loc[data_ind]
...
class DATASET_GCN_W_FT(Dataset):
	...
	cell_basal, gcn_adj, gcn_adj_weight, 
	cell_info, syn_ans ):
...
def make_merged_data(CV) :
	...
	T_train = DATASET_GCN_W_FT(
		...
		torch.Tensor(train_data['cell_BASAL']), 
	...
	T_val = DATASET_GCN_W_FT(
		...
		torch.Tensor(val_data['cell_BASAL']), 
		JY_ADJ_IDX, JY_IDX_WEIGHT_T, 
		val_data['cell'].float(),
	...
	T_test = DATASET_GCN_W_FT(
		...
		torch.Tensor(test_data['drug1_adj']), torch.Tensor(test_data['drug2_adj']),
		 torch.Tensor(test_data['cell_BASAL']), 
		JY_ADJ_IDX, JY_IDX_WEIGHT_T, 
	...
	def forward(self, Drug1_F, Drug2_F, Drug1_ADJ, Drug2_ADJ, EXP1, EXP2, EXP_ADJ, EXP_ADJ_WGT, cell, syn ):
		...
		return X
										...
										CV_0_MODEL = MY_expGCN_parallel_model(
													3, CV_0_train.gcn_drug1_F.shape[-1] , 16,      # G_layer_chem, G_indim_chem, G_hiddim_chem, 
													3, 1 , 32,      # G_layer_exp, G_indim_exp, G_hiddim_exp, 
													[128, 128, 128], [128, 128, 128], [32,32],      # drug 1 layers, drug 2 layers, merged layer, 
													len(set(A_B_C_S_SET_SM.DrugCombCCLE)), 1,      # cell_dim ,out_dim,
													0.5, 0.5      # inDrop, drop
													)

							CV_0_MODEL(drug1_f, drug2_f, drug1_a, drug2_a, expA, expB, adj, adj_w, cell, y)
...
W_NAME = 'W24'
WORK_PATH = '/home01/k020a01/02.M3V5/M3V5_W24_349/'

MJ_NAME = 'M3V5'
WORK_DATE = '23.04.23' # 349
MISS_NAME = 'MIS2'
PPI_NAME = '349'
