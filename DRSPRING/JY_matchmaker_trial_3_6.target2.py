...
WORK_PATH = '/home01/k006a01/PRJ.01/TRIAL_3.6/'
...
# TARGET # 
생각해보니 지금까지 좀 잘못한게, CID 마다 target 이 없으면 그냥 0 으로 표시해도 되는일임 
흐미 난 바보... sample 이 줄어들 필요가 없었음 

TARGET_raw_DB = pd.read_csv(TARGET_PATH+'combined_target_b_woprediction.csv', low_memory=False)
TARGET_raw_DB_C = pd.read_csv(TARGET_PATH+'cid_target_dbcount_b_woprediction.csv', low_memory=False)

TARGET_no_3 = pd.read_csv(TARGET_PATH+'combined_target.csv', low_memory=False) # 원래 쓰던거 
TARGET_no_3_C = pd.read_csv(TARGET_PATH+'cid_target_dbcount.csv', low_memory=False) # 원래 쓰던거 

TARGET_no_4 = pd.read_csv(TARGET_PATH+'combined_target_wcbsi.csv', low_memory=False)
TARGET_no_4_C = pd.read_csv(TARGET_PATH+'cid_target_dbcount_wcbsi.csv', low_memory=False)
...
# (1) total target only over 2 
TARGET_DB = TARGET_raw_DB_C
T_NAME = 'T1'


# (2) target without BD,ST,SP only over 2 
TARGET_DB = TARGET_no_3_C
T_NAME = 'T2'


# (3) target without BD,ST,SP,CTD only over 2 
TARGET_DB = TARGET_no_4_C
T_NAME = 'T3'
...
# drug target filter -> 그럴 필요가 없을지도 
...
A_B_C_S_SET = A_B_C_S_SET.reset_index() # 9230 
...
A_B_C_S_SET = A_B_C_S_SET.reset_index(drop=True) # 8856
...
# edited version (first : if nothing matches)
def get_targets(sig_id): # [X, 0, 1]
...
torch.save(MY_chem_A_feat, WORK_PATH+'0812.{}.MY_chem_A_feat.pt'.format(T_NAME))
torch.save(MY_chem_B_feat, WORK_PATH+'0812.{}.MY_chem_B_feat.pt'.format(T_NAME))
torch.save(MY_chem_A_adj, WORK_PATH+'0812.{}.MY_chem_A_adj.pt'.format(T_NAME))
torch.save(MY_chem_B_adj, WORK_PATH+'0812.{}.MY_chem_B_adj.pt'.format(T_NAME))
torch.save(MY_exp_A, WORK_PATH+'0812.{}.MY_exp_A.pt'.format(T_NAME))
torch.save(MY_exp_B, WORK_PATH+'0812.{}.MY_exp_B.pt'.format(T_NAME))
torch.save(MY_exp_AB, WORK_PATH+'0812.{}.MY_exp_AB.pt'.format(T_NAME))
torch.save(MY_tgt_A, WORK_PATH+'0812.{}.MY_tgt_A.pt'.format(T_NAME))
torch.save(MY_tgt_B, WORK_PATH+'0812.{}.MY_tgt_B.pt'.format(T_NAME))
torch.save(MY_syn, WORK_PATH+'0812.{}.MY_syn.pt'.format(T_NAME))
...
# 세개 결과를 빨리 봐야하는데.... 이번에 해보니까 100개는 너무 많은것 같고. 한 50개 정도면 되려나 
MAIN('22.08.05.PRJ01.TRIAL3_6.T1', WORK_PATH, 50, 1000, 150, 32, 1)
MAIN('22.08.05.PRJ01.TRIAL3_6.T2', WORK_PATH, 50, 1000, 150, 32, 1)
MAIN('22.08.05.PRJ01.TRIAL3_6.T3', WORK_PATH, 50, 1000, 150, 32, 1)
...
anal_df = ExperimentAnalysis("~/ray_results/22.08.05.PRJ01.TRIAL3_6.{}".format(G_NAME))

ANA_DF = anal_df.dataframe()
ANA_ALL_DF = anal_df.trial_dataframes

ANA_DF.to_csv('/home01/k006a01/PRJ.01/TRIAL_3.6/RAY_ANA_DF.P01.3_6.{}.csv'.format(G_NAME))
import pickle
with open("/home01/k006a01/PRJ.01/TRIAL_3.6/RAY_ANA_DF.P01.3_6.{}.pickle".format(G_NAME), "wb") as fp:
...
# get /home01/k006a01/ray_results/22.08.05.PRJ01.TRIAL3_6.T1/RAY_MY_train_d3b08200_26_G_hiddim=128,G_layer=2,batch_size=32,dropout_1=0.5000,dropout_2=0.2000,epoch=1000,feat_size_0=1024,feat_s_2022-08-06_02-14-52/model.pth T1.M1_model.pth
# get /home01/k006a01/ray_results/22.08.05.PRJ01.TRIAL3_6.T2/RAY_MY_train_c268a260_20_G_hiddim=64,G_layer=4,batch_size=64,dropout_1=0.2000,dropout_2=0.0100,epoch=1000,feat_size_0=64,feat_size_2022-08-06_21-46-23/model.pth T2.M1_model.pth
# get /home01/k006a01/ray_results/22.08.05.PRJ01.TRIAL3_6.T3/RAY_MY_train_bbded722_23_G_hiddim=64,G_layer=3,batch_size=32,dropout_1=0.2000,dropout_2=0.2000,epoch=1000,feat_size_0=64,feat_size_2022-08-07_09-48-07/model.pth T3.M1_model.pth
...
# get /home01/k006a01/ray_results/22.08.05.PRJ01.TRIAL3_6.T1/RAY_MY_train_d3b08200_26_G_hiddim=128,G_layer=2,batch_size=32,dropout_1=0.5000,dropout_2=0.2000,epoch=1000,feat_size_0=1024,feat_s_2022-08-06_02-14-52/checkpoint_000766/checkpoint T1.M2_checkpoint
# get /home01/k006a01/ray_results/22.08.05.PRJ01.TRIAL3_6.T2/RAY_MY_train_c268a260_20_G_hiddim=64,G_layer=4,batch_size=64,dropout_1=0.2000,dropout_2=0.0100,epoch=1000,feat_size_0=64,feat_size_2022-08-06_21-46-23/checkpoint_000918/checkpoint T2.M2_checkpoint
# get /home01/k006a01/ray_results/22.08.05.PRJ01.TRIAL3_6.T3/RAY_MY_train_bbded722_23_G_hiddim=64,G_layer=3,batch_size=32,dropout_1=0.2000,dropout_2=0.2000,epoch=1000,feat_size_0=64,feat_size_2022-08-07_09-48-07/checkpoint_000907/checkpoint T3.M2_checkpoint
...
# get /home01/k006a01/ray_results/22.08.05.PRJ01.TRIAL3_6.T1/RAY_MY_train_d3b08200_26_G_hiddim=128,G_layer=2,batch_size=32,dropout_1=0.5000,dropout_2=0.2000,epoch=1000,feat_size_0=1024,feat_s_2022-08-06_02-14-52/checkpoint_000766/checkpoint T1.M4_checkpoint
# get /home01/k006a01/ray_results/22.08.05.PRJ01.TRIAL3_6.T2/RAY_MY_train_c268a260_20_G_hiddim=64,G_layer=4,batch_size=64,dropout_1=0.2000,dropout_2=0.0100,epoch=1000,feat_size_0=64,feat_size_2022-08-06_21-46-23/checkpoint_000918/checkpoint T2.M4_checkpoint
# get /home01/k006a01/ray_results/22.08.05.PRJ01.TRIAL3_6.T3/RAY_MY_train_bbded722_23_G_hiddim=64,G_layer=3,batch_size=32,dropout_1=0.2000,dropout_2=0.2000,epoch=1000,feat_size_0=64,feat_size_2022-08-07_09-48-07/checkpoint_000907/checkpoint T3.M4_checkpoint
...
WORK_PATH = '/st06/jiyeonH/11.TOX/MY_TRIAL_GPU/PJ01.TRIAL.3_6/'
...
MY_chem_A_feat = torch.load(WORK_PATH+'0805.{}.MY_chem_A_feat.pt'.format(G_NAME))
MY_chem_B_feat = torch.load(WORK_PATH+'0805.{}.MY_chem_B_feat.pt'.format(G_NAME))
MY_chem_A_adj = torch.load(WORK_PATH+'0805.{}.MY_chem_A_adj.pt'.format(G_NAME))
MY_chem_B_adj = torch.load(WORK_PATH+'0805.{}.MY_chem_B_adj.pt'.format(G_NAME))
MY_exp_A = torch.load(WORK_PATH+'0805.{}.MY_exp_A.pt'.format(G_NAME))
MY_exp_B = torch.load(WORK_PATH+'0805.{}.MY_exp_B.pt'.format(G_NAME))
MY_exp_AB = torch.load(WORK_PATH+'0805.{}.MY_exp_AB.pt'.format(G_NAME))
MY_tgt_A = torch.load(WORK_PATH+'0805.{}.MY_tgt_A.pt'.format(G_NAME))
MY_tgt_B = torch.load(WORK_PATH+'0805.{}.MY_tgt_B.pt'.format(G_NAME))
MY_syn = torch.load(WORK_PATH+'0805.{}.MY_syn.pt'.format(G_NAME))
...
PRJ_PATH = '/st06/jiyeonH/11.TOX/MY_TRIAL_GPU/PJ01.TRIAL.3_6/'
ANA_DF = pd.read_csv(PRJ_PATH+'RAY_ANA_DF.P01.3_6.{}.csv'.format(G_NAME))
with open(PRJ_PATH+'RAY_ANA_DF.P01.3_6.{}.pickle'.format(G_NAME), 'rb') as f:
...
PRJ_PATH, 'TRIAL.3_6.{}.BEST.loss'.format(G_NAME) )
...
R_1_T = TEST_LOSS
R_1_1 , R_1_2 = jy_corrplot(PRED_list, Y_list, PRJ_PATH,'P3_6.{}.M1_model'.format(G_NAME) )
...
R_2_T = TEST_LOSS
R_2_1 , R_2_2 = jy_corrplot(PRED_list, Y_list,PRJ_PATH,'P3_6.{}.M2_checkpoint'.format(G_NAME) )
...
mini_df = ANA_ALL_DF[TOT_key]
plot_loss(list(mini_df.TrainLoss), list(mini_df.ValLoss), 
PRJ_PATH, 'TRIAL.3_6.{}.MIN.loss'.format(G_NAME) )
...
R_3_T = TEST_LOSS
R_3_1 , R_3_2 = jy_corrplot(PRED_list, Y_list,PRJ_PATH,'P3_6.{}.M4_checkpoint'.format(G_NAME) )
