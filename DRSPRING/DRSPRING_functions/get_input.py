...
def make_merged_data(train_data, val_data, test_data, JY_ADJ_IDX, JY_IDX_WEIGHT_T) :
	#train_data = globals()['train_data_'+str(CV)]
...
def prepare_input(MJ_NAME, WORK_DATE, MISS_NAME, file_name, WORK_NAME, CELL_CUT):
	...
	#
	#
	LOSS_WEIGHT_0 = get_loss_weight(train_data)
	JY_IDX_WEIGHT_T = torch.Tensor(JY_IDX_WEIGHT).view(1,-1)
	#
	#
	T_train_0, T_val_0, T_test_0 = make_merged_data(train_data, val_data, test_data, JY_ADJ_IDX, JY_IDX_WEIGHT_T)
	batch_cut_weight = LOSS_WEIGHT_0[0:64]
	#
	return A_B_C_S_SET_SM, T_train_0, T_val_0, T_test_0, batch_cut_weight




#왜 index 이름 안맞는지 확인해야함 
