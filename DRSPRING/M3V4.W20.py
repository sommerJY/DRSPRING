...
C_df = C_df.sort_values('freq')
									...
									path = '/st06/jiyeonH/11.TOX/DR_SPRING/trials/M3V4_MIS2_W20/'
...
# 10으로 나눈 이유는 nested CV 가 가능하지 않을까 싶어서 그랬으나... 음 .... 살짝 포기하기로 함
# 일단 아래 코드를 진행하는건... 애들별로 나누고 싶어서임. 
# 8:1:1 이었는데 80:20 으로 train+val : test 나누고
# 다시 그 안에서 5 CV 로 나눌 생각을 해야하는데, 리를빗 피곤해지네? 
# 어쨌든 그래서 그룹을 두번으로 나누기로 함 

# to split TrainVal and Test -> 
# 사실 이렇게 안해도 되지만 그냥.. 
...
# 그럼에도 불구하고 
# 이번부터 CV 를 사용하려고 하는데 약간 헷갈리는게 아예 본적이 없는 test 여야 하는건지, 그리고 CV 사이에 weight reset 이 필요한건지 
# 그럼 이번에 딱 결정하자
# 나의 TEST SET 은 CV_9_list 랑 CV_10_list 이야
# 다른 사람들도 hyperparameter tuning 과 ray 를 어떻게 써보려고 했으나 실패한걸로 보임.
# 찾아보니까 내가 원하는 궁극적인 결과는 nested cross validation 인데, 그걸 성공한 ray 가 없을뿐더러 
# 생각해봐도 cv loop 을 두번 돌리는거니까 좀 에바이긴 함. 안그래도 계산할게 오지게 많은데?
# 그래서 그냥 기본적인 cross validation 으로 돌아가기로 함 ㅎ 

# 다시 생각해보니까 tuning 하는 이유 자체는 원래 좋은 hyperparameter 를 찾기 위한 일인거고 
# cross validation 은 hyperparameter 가 fix 된 상태에서 Cross validation 을 이용해 generalized 값을 얻고자 하는게 목표임. 
# 그래서 ray 에 굳이굳이 적용을 해보려면 
# hyper parameter tuning 을 넣은 모델을 기준으로 그 안에서 test 만 고정시키고 나머지에서만 5CV 후 각 5CV 값을 저장하는 느낌으로다가?
# 그렇게 되면 걱정은, 음,,, 
# cross validation 의 역할은 데이터에 따라서 모델 수치가 움직이게 하지 않으려는게 목표니까 매 CV 때마다 모델이 새로 값을 부여받아야하는거 아닌가
# 그럼 model weight 을 어떻게 다시 저장하지? 
# 아 이해했음. 
# ray 를 통해서 만들어주는 hyperparameter model 예시가 있으면 걔를 여러번 돌려주면 되겠네.
# 그럼 그때 그때 모델을 같이 저장해줘야하는건가? (저장공간이 더 필요해지나?)

# 아 이거만 넣고 그림 그려야해 -> 글도 써야하고 

# 그으래서 난 대충 8:1:1 이니까
# 

																			CV_ND_INDS = {
																				'CV0_train' : CV_1_list+ CV_2_list+CV_3_list+CV_4_list+ CV_5_list+CV_6_list+CV_7_list+CV_8_list, 
																				'CV0_val' : CV_9_list, 'CV0_test' : CV_10_list,
																				'CV1_train' : CV_3_list+CV_4_list+ CV_5_list+CV_6_list+CV_7_list+CV_8_list+CV_9_list+CV_10_list, 
																				'CV1_val' : CV_1_list,'CV1_test' : CV_2_list,
																				'CV2_train' : CV_5_list+CV_6_list+CV_7_list+CV_8_list+CV_9_list+CV_10_list+CV_1_list+ CV_2_list, 
																				'CV2_val' : CV_3_list,'CV2_test' : CV_4_list,
																				'CV3_train' : CV_7_list+CV_8_list+CV_9_list+CV_10_list+CV_1_list+ CV_2_list+CV_3_list+CV_4_list, 
																				'CV3_val' : CV_5_list,'CV3_test' : CV_6_list,
																				'CV4_train' : CV_9_list+CV_10_list+CV_1_list+ CV_2_list+CV_3_list+CV_4_list+CV_5_list+CV_6_list, 
																				'CV4_val' : CV_7_list,'CV4_test' : CV_8_list 
																			}
...
아 지금 약간 CV 적용할려고 했다가 도랑에 빠져버렸고요
그러면 1000 epoch 을 돌리긴 돌릴건데,
CV 를 또 적용하려면 training 모델을 다시 시작해서 다시 러닝을 시켜줘야하는거임.
그럼 나중에 어떤 애를 가져다가 test 에 따로 사용해야할지 또 모르겠넴 
성능에 목맸다가 이런 결과가 나왔구요
그러면 그러면 
