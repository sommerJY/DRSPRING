...
hunet_pw = pd.read_csv(hunet_dir+'HS-DB.tsv', sep = '\t', header = None)
hunet_pw.columns = ['G_A','G_B','score']

L_matching_list = pd.read_csv(IDK_PATH+'L_12_string.csv', sep = '\t')

hnet_IAS_L1 = hunet_pw[hunet_pw['G_A'].isin(L_matching_list.entrez)]
...
IAS_FILTER = copy.deepcopy(hunet_pw)
...
torch.save(MY_chem_A_feat, WORK_PATH+'0802.{}.MY_chem_A_feat.pt'.format(G_NAME))
torch.save(MY_chem_B_feat, WORK_PATH+'0802.{}.MY_chem_B_feat.pt'.format(G_NAME))
torch.save(MY_chem_A_adj, WORK_PATH+'0802.{}.MY_chem_A_adj.pt'.format(G_NAME))
torch.save(MY_chem_B_adj, WORK_PATH+'0802.{}.MY_chem_B_adj.pt'.format(G_NAME))
torch.save(MY_exp_A, WORK_PATH+'0802.{}.MY_exp_A.pt'.format(G_NAME))
torch.save(MY_exp_B, WORK_PATH+'0802.{}.MY_exp_B.pt'.format(G_NAME))
torch.save(MY_exp_AB, WORK_PATH+'0802.{}.MY_exp_AB.pt'.format(G_NAME))
torch.save(MY_tgt_A, WORK_PATH+'0802.{}.MY_tgt_A.pt'.format(G_NAME))
torch.save(MY_tgt_B, WORK_PATH+'0802.{}.MY_tgt_B.pt'.format(G_NAME))
torch.save(MY_syn, WORK_PATH+'0802.{}.MY_syn.pt'.format(G_NAME))
...
MAIN('22.08.02.PRJ01.TRIAL3_4.{}'.format(G_NAME), WORK_PATH, 6, 1000, 600, 32, 1)
