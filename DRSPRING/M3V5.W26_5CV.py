...
# 일단 cell vector 제거 버전 -> data 재정의 필요 없음 
...
class MY_expGCN_parallel_model(torch.nn.Module):
	...
	out_dim, inDrop, drop):
		...
		#
		self.SNPs = torch.nn.ModuleList([torch.nn.Linear(self.layers_1[-1]+self.layers_2[-1] , self.layers_3[0] )]) ##### 바꿨음! 
	...
	def forward(self, Drug1_F, Drug2_F, Drug1_ADJ, Drug2_ADJ, EXP1, EXP2, EXP_ADJ, EXP_ADJ_WGT, cell, syn ):
		...
		for L2 in range(len(self.Convs_2)):
			if L2 != len(self.Convs_2)-1 :
				...
				input_drug2 = self.Convs_2[L2](input_drug2)
		#
		X = torch.cat(( input_drug1, input_drug2 ), 1) ################# 제거했음 
...
WORK_PATH = '/home01/k020a01/02.M3V5/M3V5_W26_349_MIS2'
PPI_NAME = '349'
...
sbatch gpu8.any M3V5_WORK26.349.py

tail ~/02.M3V5/M3V5_W26_349_MIS2/RESULT.G8.txt

tail ~/logs/M3V5W26_GPU8_11270.log





# 결과가 진짜 영구린데, 좀 그러면 확인해봐야할게, 
# cell line vector 추가하기 전에 나오는 값들의 추이가 어떤지 봐야할듯
# W21 가지고 진행해야함 
# 근데 그렇게 되면 중간에 normalize 값을 넣어줘야하는건가 고민 
...
# PRJ02.23.05.11.M3V5.WORK_26.349.MIS2
...
# /st06/jiyeonH/11.TOX/DR_SPRING/trials/M3V5_W26_349_MIS2/
