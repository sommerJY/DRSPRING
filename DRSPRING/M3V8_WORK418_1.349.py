...
MY_syn_RE2 ) : 
	#
	ABCS_train = A_B_C_S_SET_SM[A_B_C_S_SET_SM.DC_cellname != CELL ]
	ABCS_test = A_B_C_S_SET_SM[A_B_C_S_SET_SM.DC_cellname == CELL ]
...
def RAY_MY_train(config, checkpoint_dir=None):
	...
	CV_NUM = config["CV"] 
	# tissue & CID 다른거 확인하기 !!!!
	RAY_train_list = [RAY_train_0, RAY_train_1, RAY_train_2, RAY_train_3, RAY_train_4, RAY_train_5, RAY_train_6, RAY_train_7] # 
	RAY_test_list = [RAY_test_0, RAY_test_1, RAY_test_2, RAY_test_3, RAY_test_4, RAY_test_5, RAY_test_6, RAY_test_7] # 
	RAY_loss_weight_list = [RAY_loss_weight_0, RAY_loss_weight_1, RAY_loss_weight_2, RAY_loss_weight_3, RAY_loss_weight_4, RAY_loss_weight_5, RAY_loss_weight_6, RAY_loss_weight_7] # 
	#
	# 
	...
	#
	CV_0_loaders = {
			'train' : torch.utils.data.DataLoader(CV_0_train, batch_size = int(config["batch_size"]), collate_fn = graph_collate_fn, shuffle =False),# , num_workers=config['n_workers']
			'test' : torch.utils.data.DataLoader(CV_0_test, batch_size = int(config["batch_size"]), collate_fn = graph_collate_fn, shuffle =False), # , num_workers=config['n_workers']
	...
	for epoch in range(n_epochs):
		...
		# train the model #
		###################
		cv_0_t_loss, cv_0_t_pc, cv_0_t_sc, CV_0_MODEL, CV_0_optimizer  = inner_train(CV_0_loaders, CV_0_MODEL, CV_0_optimizer, False)
		...
		# validate the model #
		######################
		cv_0_v_loss, cv_0_v_pc, cv_0_v_sc, CV_0_MODEL  = inner_val(CV_0_loaders, CV_0_MODEL, False)
...
def MAIN(ANAL_name, my_config, num_samples= 10, max_num_epochs=1000, cpus_per_trial = 16, gpus_per_trial = 1):
	...
	CONFIG={
		...
		# !!!!!!!!!!!!
		#
		"G_chem_layer" : tune.grid_search([my_config['config/G_chem_layer']]), # 
		"G_exp_layer" : tune.grid_search([my_config['config/G_exp_layer']]), # 
		"G_chem_hdim" : tune.grid_search([my_config['config/G_chem_hdim']]), # 
		"G_exp_hdim" : tune.grid_search([my_config['config/G_exp_hdim']]), # 
		"batch_size" : tune.grid_search([my_config['config/batch_size'] ]), # 
		"dsn_layer" : tune.grid_search([my_config['config/dsn_layer'] ]), # 
		"snp_layer" : tune.grid_search([my_config['config/snp_layer'] ]), # 
		"dropout_1" : tune.grid_search([ my_config['config/dropout_1'] ]),
		"dropout_2" : tune.grid_search([ my_config['config/dropout_2'] ]),
		"lr" : tune.grid_search([ my_config['config/lr'] ]), 
	}
	#
...
WORK_PATH = '/st06/jiyeonH/11.TOX/DR_SPRING/trials/{}_{}_{}_{}/'.format(MJ_NAME,W_NAME,PPI_NAME,MISS_NAME)
...
#4gpu for 5cv 
MAIN('PRJ02.{}.{}.{}.{}.{}'.format(WORK_DATE, MJ_NAME, WORK_NAME, PPI_NAME, MISS_NAME), my_config, 1, 1000,64,1)
