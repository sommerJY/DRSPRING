...
민지가 주는 PRJ2_EXP_ccle_all_fugcn_hhhdt3_tvt.csv 로 진행 
prep 부터 다시 확인하고 넘어가자 
...
all(BETA_BIND_ADDre.id == BETA_BIND_ORI.id) # true

tmp = list(BETA_BIND_ADD.columns)
re_col = [a for a in tmp if ':' in a]
re_col= list(set(re_col) - set(BETA_BIND_ORI.columns))

BETA_BIND_ADDre = BETA_BIND_ADD[re_col]



BETA_BIND = pd.concat([BETA_BIND_ORI, BETA_BIND_ADDre], axis =1 )
...
LINCS_ALL_PATH = '/st06/jiyeonH/11.TOX/DR_SPRING/trials/M3V6_349_FULL/'
...
-> 이걸 다시 우아하게 진행하고 싶었으나 뭐... 저장해둔걸로 하자 
코드는 변함 없는거 확인했음 
#############################3
...
for ind in range(len(id_list)) : 
	...
	#
	if len(sig_ids) > 1 :
		sig_lincs = [BETA_BIND[sig_iii].squeeze().tolist() for sig_iii in sig_ind]
...
torch.save( Mean_exp_t, LINCS_ALL_PATH + "10_24_sig_cell_mean.0620.pt")
...
BETA_BIND_SIG_df_CID5['corr_Pmean'] = corr_Pmean
BETA_BIND_SIG_df_CID5['corr_Smean'] = corr_Smean

BETA_BIND_SIG_df_CID5.to_csv(LINCS_ALL_PATH + '10_24_sig_cell_mean.0620.csv', index = False)
...
# 3146
											...
											DC_ROWleng = pd.merge(DC_ccle_final_dup, DC_DRUG_DF_FULL_leng, on ='ROW_CAN_SMILES', how = 'left')

											DC_DRUG_DF_FULL_leng.columns = ['COL_CAN_SMILES','COL_len']
											DC_COLleng = pd.merge(DC_ROWleng, DC_DRUG_DF_FULL_leng, on ='COL_CAN_SMILES', how = 'left')

											DC_COLleng['ROW_len'] = [int(a) for a in DC_COLleng.ROW_len]
											DC_COLleng['COL_len'] = [int(a) for a in DC_COLleng.COL_len]

											DC_rlen = DC_COLleng[DC_COLleng.ROW_len<=50]
											DC_clen = DC_rlen[DC_rlen.COL_len<=50]
...
# A_B_C_S.to_csv('/st06/jiyeonH/11.TOX/DR_SPRING/trials/M3V6_349_FULL/A_B_C_S.6020.csv', sep = '\t', index= False)
# A_B_C_S = pd.read_csv('/st06/jiyeonH/11.TOX/DR_SPRING/trials/M3V6_349_FULL/A_B_C_S.6020.csv', sep = '\t', low_memory = False)
