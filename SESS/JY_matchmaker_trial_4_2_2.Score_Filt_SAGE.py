...
WORK_PATH = '/home01/k006a01/PRJ.01/TRIAL_4.2.2/'
...
def check_synergy_data(DrugA_SIG, DrugB_SIG, Cell) :
	ABCS1 = A_B_C_S[A_B_C_S.BETA_sig_id_x == DrugA_SIG]
	ABCS2 = ABCS1[ABCS1.BETA_sig_id_y == DrugB_SIG]
	ABCS3 = ABCS2[ABCS2.DrugCombCello == Cell]
	std_score = np.std(ABCS3.synergy_loewe)
	min_score = min(ABCS3.synergy_loewe)
	max_score = max(ABCS3.synergy_loewe)
	if min_score*max_score >=0 :
		check = 'True'
	else :
		check = 'False'
	return std_score, check
...
for i in range(A_B_C_S_SET.shape[0]):
	drugA = list(A_B_C_S_SET.BETA_sig_id_x)[i]
	drugB = list(A_B_C_S_SET.BETA_sig_id_y)[i]
	Cell = list(A_B_C_S_SET.DrugCombCello)[i]
	std_score, check = check_synergy_data(drugA, drugB, Cell)
	synergy_std.append(std_score)
	synergy_check.append(check)


A_B_C_S_SET['SYN_std'] = synergy_std
A_B_C_S_SET['SYN_log'] = np.log([a+0.00001 for a in synergy_std])
A_B_C_S_SET['SYN_cck'] = synergy_check
#A_B_C_S_SET['cat'] = "syn"



#분포 확인 
#plot_dir = '/st06/jiyeonH/11.TOX/MY_TRIAL_6/'
#import seaborn 
#sns.set_context('talk', font_scale=1.1)
#plt.figure(figsize=(8,6))
#sns.violinplot(y = 'SYN_std', x= 'cat', data =A_B_C_S_SET, color = '#cde7ff', inner= 'quartile')
#sns.stripplot(y = 'SYN_std', x = 'cat', data = A_B_C_S_SET, hue = 'SYN_cck', edgecolor= 'gray', palette ={'True':'#444444', 'False':'#e83298' }, jitter =True, alpha = 0.5)
#plt.savefig(plot_dir+"Violin_synscore.png", format = 'png', dpi=300)
#음..
...
#sns.set_context('talk', font_scale=1.1)
#plt.figure(figsize=(8,6))
#sns.violinplot(y = 'SYN_std', x= 'SYN_cck', data =A_B_C_S_SET, color = '#cde7ff', inner= 'quartile')
#sns.stripplot(y = 'SYN_std', x = 'SYN_cck', data = A_B_C_S_SET, hue = 'SYN_cck', edgecolor= 'gray', palette ={'True':'#444444', 'False':'#e83298' }, jitter =True, alpha = 0.5)
#plt.savefig(plot_dir+"Violin_synscore2.png", format = 'png', dpi=300)
...
#true_abc = A_B_C_S_SET[A_B_C_S_SET.SYN_cck == 'True']
#false_abc = A_B_C_S_SET[A_B_C_S_SET.SYN_cck == 'False']

#true_hist = plt.hist( true_abc['SYN_std'], range = (0, 33), bins = 10)
#flase_hist = plt.hist( false_abc['SYN_std'], range = (0, 33), bins = 10)

#x_name = list(np.round(true_hist[1],2))
#x_name = ["~"+str(a) for a in x_name ]
#x_name = x_name[1:]

#fig, ax = plt.subplots(figsize=(10,8))
#ax.bar(x_name, list(true_hist[0]), width = 0.5, label = 'True', color = '#444444')
#ax.bar(x_name, list(flase_hist[0]),bottom=list(true_hist[0]), width = 0.5, label = 'False', color = '#e83298')
#ax.legend()
#plt.savefig(plot_dir+"Hist_synscore.png", format = 'png', dpi=300)
...
A_B_C_S_SET = A_B_C_S_SET[A_B_C_S_SET.SYN_cck == 'True']
...
torch.save(MY_chem_A_feat, WORK_PATH+'0923.{}.MY_chem_A_feat.pt'.format(G_NAME))
torch.save(MY_chem_B_feat, WORK_PATH+'0923.{}.MY_chem_B_feat.pt'.format(G_NAME))
torch.save(MY_chem_A_adj, WORK_PATH+'0923.{}.MY_chem_A_adj.pt'.format(G_NAME))
torch.save(MY_chem_B_adj, WORK_PATH+'0923.{}.MY_chem_B_adj.pt'.format(G_NAME))
torch.save(MY_exp_A, WORK_PATH+'0923.{}.MY_exp_A.pt'.format(G_NAME))
torch.save(MY_exp_B, WORK_PATH+'0923.{}.MY_exp_B.pt'.format(G_NAME))
torch.save(MY_exp_AB, WORK_PATH+'0923.{}.MY_exp_AB.pt'.format(G_NAME))
torch.save(MY_Cell, WORK_PATH+'0923.{}.MY_Cell.pt'.format(G_NAME))
torch.save(MY_tgt_A, WORK_PATH+'0923.{}.MY_tgt_A.pt'.format(G_NAME))
torch.save(MY_tgt_B, WORK_PATH+'0923.{}.MY_tgt_B.pt'.format(G_NAME))
torch.save(MY_syn, WORK_PATH+'0923.{}.MY_syn.pt'.format(G_NAME))
...
# local
MAIN('22.09.23.PRJ01.TRIAL4_2_2', WORK_PATH, WORK_PATH, '4_2_2', 'score_sage', 3, 3, 1, 32, 1)

# 4gpu
MAIN('22.09.23.PRJ01.TRIAL4_2_2', WORK_PATH, WORK_PATH, '4_2_2', 'score_sage', 100, 1000, 150, 32, 1)

# 8gpu
MAIN('22.09.23.PRJ01.TRIAL4_2_2', WORK_PATH, WORK_PATH, '4_2_2', 'score_sage', 100, 1000, 150, 16, 1)
...
anal_df = ExperimentAnalysis("~/ray_results/22.09.23.PRJ01.TRIAL4_2_2")
...
ANA_DF.to_csv('/home01/k006a01/PRJ.01/TRIAL_4.2.2/RAY_ANA_DF.P01.4_2_2.csv')
import pickle
with open("/home01/k006a01/PRJ.01/TRIAL_4.2.2/RAY_ANA_DF.P01.4_2_2.pickle", "wb") as fp:
...
DF_KEY = list(ANA_DF.sort_values('ValLoss')['logdir'])[0] # 131
DF_KEY
# get /home01/k006a01/ray_results/22.09.23.PRJ01.TRIAL4_2_2/RAY_MY_train_7ceb2288_70_G_hiddim=32,G_layer=4,batch_size=16,dropout_1=0.2000,dropout_2=0.0100,epoch=1000,feat_size_0=128,feat_siz_2022-09-24_21-37-44/model.pth M1_model.pth
...
cck_num =mini_df[mini_df.ValLoss==min(mini_df.ValLoss)].index.item() # 130
checkpoint = "/checkpoint_"+str(cck_num).zfill(6)
TOPVAL_PATH = DF_KEY + checkpoint
TOPVAL_PATH

# get /home01/k006a01/ray_results/22.09.23.PRJ01.TRIAL4_2_2/RAY_MY_train_7ceb2288_70_G_hiddim=32,G_layer=4,batch_size=16,dropout_1=0.2000,dropout_2=0.0100,epoch=1000,feat_size_0=128,feat_siz_2022-09-24_21-37-44/checkpoint_000887/checkpoint M2_checkpoint
...
# 130
# get /home01/k006a01/ray_results/22.09.23.PRJ01.TRIAL4_2_2/RAY_MY_train_7ceb2288_70_G_hiddim=32,G_layer=4,batch_size=16,dropout_1=0.2000,dropout_2=0.0100,epoch=1000,feat_size_0=128,feat_siz_2022-09-24_21-37-44/checkpoint_000887/checkpoint M4_checkpoint
...
WORK_PATH = '/st06/jiyeonH/11.TOX/MY_TRIAL_GPU/PJ01.TRIAL.4_2_2/'


G_NAME='s_filt_sage'

MY_chem_A_feat = torch.load(WORK_PATH+'0923.{}.MY_chem_A_feat.pt'.format(G_NAME))
MY_chem_B_feat = torch.load(WORK_PATH+'0923.{}.MY_chem_B_feat.pt'.format(G_NAME))
MY_chem_A_adj = torch.load(WORK_PATH+'0923.{}.MY_chem_A_adj.pt'.format(G_NAME))
MY_chem_B_adj = torch.load(WORK_PATH+'0923.{}.MY_chem_B_adj.pt'.format(G_NAME))
MY_exp_A = torch.load(WORK_PATH+'0923.{}.MY_exp_A.pt'.format(G_NAME))
MY_exp_B = torch.load(WORK_PATH+'0923.{}.MY_exp_B.pt'.format(G_NAME))
MY_exp_AB = torch.load(WORK_PATH+'0923.{}.MY_exp_AB.pt'.format(G_NAME))
MY_Cell = torch.load(WORK_PATH+'0923.{}.MY_Cell.pt'.format(G_NAME))
MY_tgt_A = torch.load(WORK_PATH+'0923.{}.MY_tgt_A.pt'.format(G_NAME))
MY_tgt_B = torch.load(WORK_PATH+'0923.{}.MY_tgt_B.pt'.format(G_NAME))
MY_syn = torch.load(WORK_PATH+'0923.{}.MY_syn.pt'.format(G_NAME))
...
PRJ_PATH = '/st06/jiyeonH/11.TOX/MY_TRIAL_GPU/PJ01.TRIAL.4_2_2/'
ANA_DF = pd.read_csv(PRJ_PATH+'RAY_ANA_DF.P01.4_2_2.csv')
with open(PRJ_PATH+'RAY_ANA_DF.P01.4_2_2.pickle', 'rb') as f:
