...
G_NAME='VAR_1' # Attention 
G_NAME='VAR_2' # GraphSage
...
MY_chem_A_feat = torch.empty(size=(128, 50, 64))
MY_chem_B_feat= torch.empty(size=(128, 50, 64))
MY_chem_A_adj = torch.empty(size=(128, 50, 50))
MY_chem_B_adj= torch.empty(size=(128, 50, 50))
...
torch.save(MY_chem_A_feat, WORK_PATH+'0831.{}.MY_chem_A_feat.pt'.format(G_NAME))
torch.save(MY_chem_B_feat, WORK_PATH+'0831.{}.MY_chem_B_feat.pt'.format(G_NAME))
torch.save(MY_chem_A_adj, WORK_PATH+'0831.{}.MY_chem_A_adj.pt'.format(G_NAME))
torch.save(MY_chem_B_adj, WORK_PATH+'0831.{}.MY_chem_B_adj.pt'.format(G_NAME))
torch.save(MY_exp_A, WORK_PATH+'0831.{}.MY_exp_A.pt'.format(G_NAME))
torch.save(MY_exp_B, WORK_PATH+'0831.{}.MY_exp_B.pt'.format(G_NAME))
torch.save(MY_exp_AB, WORK_PATH+'0831.{}.MY_exp_AB.pt'.format(G_NAME))
torch.save(MY_Cell, WORK_PATH+'0831.{}.MY_Cell.pt'.format(G_NAME))
torch.save(MY_tgt_A, WORK_PATH+'0831.{}.MY_tgt_A.pt'.format(G_NAME))
torch.save(MY_tgt_B, WORK_PATH+'0831.{}.MY_tgt_B.pt'.format(G_NAME))
torch.save(MY_syn, WORK_PATH+'0831.{}.MY_syn.pt'.format(G_NAME))
...
class MY_expGCN_parallel_model(torch.nn.Module):
	def __init__(self, G_layer_chem, G_indim_chem, G_hiddim_chem, G_layer_exp, G_indim_exp, G_hiddim_exp, G_head, layers_1, layers_2, layers_3, cell_dim ,out_dim, inDrop, drop):
		...
		#
		self.G_convs_1_chem = torch.nn.ModuleList([pyg_nn.GATConv(self.G_indim_chem, self.G_hiddim_chem, heads = self.G_head, concat = False )])
		self.G_convs_1_chem.extend([pyg_nn.GATConv(self.G_hiddim_chem, self.G_hiddim_chem, heads = self.G_head, concat = False) for i in range(self.G_layer_chem-2)])
		self.G_convs_1_chem.extend([pyg_nn.GATConv(self.G_hiddim_chem, self.G_hiddim_chem)])
		self.G_bns_1_chem = torch.nn.ModuleList([torch.nn.BatchNorm1d(self.G_hiddim_chem) for i in range(self.G_layer_chem-1)])
		#
		self.G_convs_2_chem = torch.nn.ModuleList([pyg_nn.GATConv(self.G_indim_chem, self.G_hiddim_chem, heads = self.G_head, concat = False)])
		self.G_convs_2_chem.extend([pyg_nn.GATConv(self.G_hiddim_chem, self.G_hiddim_chem, heads = self.G_head, concat = False) for i in range(self.G_layer_chem-2)])
		...
		#
		self.G_convs_1_exp = torch.nn.ModuleList([pyg_nn.GATConv(self.G_indim_exp, self.G_hiddim_exp, heads = self.G_head, concat = False)])
		self.G_convs_1_exp.extend([pyg_nn.GATConv(self.G_hiddim_exp, self.G_hiddim_exp, heads = self.G_head, concat = False) for i in range(self.G_layer_exp-2)])
		...
		#
		self.G_convs_2_exp = torch.nn.ModuleList([pyg_nn.GATConv(self.G_indim_exp, self.G_hiddim_exp, heads = self.G_head, concat = False)])
		self.G_convs_2_exp.extend([pyg_nn.GATConv(self.G_hiddim_exp, self.G_hiddim_exp, heads = self.G_head, concat = False) for i in range(self.G_layer_exp-2)])
	...
	def forward(self, Drug1_F, Drug2_F, Drug1_ADJ, Drug2_ADJ, EXP1, EXP2, EXP_ADJ, EXP_ADJ_WGT, syn, cell ):
		...
		for G_1_E in range(len(self.G_convs_1_exp)):
			if G_1_E == len(self.G_convs_1_exp)-1 :
				EXP1, EXP1_atw = self.G_convs_1_exp[G_1_E](x=EXP1, edge_index=EXP_ADJ, return_attention_weights=True) # , edge_attr= EXP_ADJ_WGT
				Exp_weight['G_1_E_'+ str(G_1_E)] = EXP1_atw[1].cpu().detach().numpy()
		...
		for G_2_E in range(len(self.G_convs_2_exp)):
			if G_2_E == len(self.G_convs_2_exp)-1 :
				EXP2, EXP2_atw = self.G_convs_2_exp[G_2_E](x=EXP2, edge_index=EXP_ADJ, return_attention_weights=True)
				Exp_weight['G_2_E_'+ str(G_2_E)] = EXP2_atw[1].cpu().detach().numpy()
				EXP2 = F.dropout(EXP2, p=self.inDrop, training=self.training) # , edge_attr= EXP_ADJ_WGT
				EXP2 = self.pool(EXP2, Exp_batch_label )
				EXP2 = self.tanh(EXP2)
				G_2_E_out = EXP2
			else :
				EXP2, EXP2_atw = self.G_convs_2_exp[G_2_E](x=EXP2, edge_index=EXP_ADJ, return_attention_weights=True) # , edge_attr= EXP_ADJ_WGT
				Exp_weight['G_2_E_'+ str(G_2_E)] = EXP2_atw[1].cpu().detach().numpy()
...
ANAL_name = '22.08.30.PRJ01.TRIAL3_9_1_pre'
Trial_name= '3_9_1'
G_NAME= 'Var1'
num_samples= 3
max_num_epochs=3
...
ANA = MAIN('22.08.30.PRJ01.TRIAL3_9_1_pre', WORK_PATH, WORK_PATH, '3_9_1','Var1', 3, 3, 1, 10, 1)
ANA = MAIN('22.08.30.PRJ01.TRIAL3_9_1', WORK_PATH, PRJ_PATH, 100, 1000, 150, 32, 1)
...
G_hiddim_exp, G_head, dsn1_layers, dsn2_layers, snp_layers, cell_dim , out_dim, inDrop, Drop)
...
R1, R2, R3 = VAR1_ATT_model(drug1_f, drug2_f, drug1_a, drug2_a, expA, expB, adj, adj_w, y, cell)
...
anal_df = ExperimentAnalysis("~/ray_results/22.08.30.PRJ01.TRIAL3_9_1")

ANA_DF = anal_df.dataframe()
ANA_ALL_DF = anal_df.trial_dataframes

done_df = ANA_DF[ANA_DF.done == True]
done_df = done_df[done_df.iterations_since_restore < 600]
done_df[['logdir']]

for logdir in list(done_df['logdir']):
	os.system('rm {}/*Exp*.npz'.format(logdir))
	os.system('rm {}/*Drug*.npz'.format(logdir))
...
ANA_DF.to_csv('/home01/k006a01/PRJ.01/TRIAL_3.9/RAY_ANA_DF.P01.3_9_1.csv')
import pickle
with open("/home01/k006a01/PRJ.01/TRIAL_3.9/RAY_ANA_DF.P01.3_9_1.pickle", "wb") as fp:
...
# get /home01/k006a01/ray_results/22.08.30.PRJ01.TRIAL3_9_1/RAY_MY_train_76aaf602_40_G_head=3,G_hiddim=32,G_layer=3,batch_size=64,dropout_1=0.2000,dropout_2=0.2000,epoch=1000,feat_size_0=512_2022-09-01_16-40-32/checkpoint_000808/checkpoint M2_checkpoint
...
# get /home01/k006a01/ray_results/22.08.30.PRJ01.TRIAL3_9_1/RAY_MY_train_93d1c964_70_G_head=3,G_hiddim=512,G_layer=3,batch_size=32,dropout_1=0.5000,dropout_2=0.2000,epoch=1000,feat_size_0=25_2022-09-03_00-54-06/checkpoint_000531/checkpoint M4_checkpoint
...
결과 확인 
WORK_PATH = '/st06/jiyeonH/11.TOX/MY_TRIAL_GPU/PJ01.TRIAL.3_9_1/'


G_NAME= 'VAR_1'

MY_chem_A_feat = torch.load(WORK_PATH+'0830.{}.MY_chem_A_feat.pt'.format(G_NAME))
MY_chem_B_feat = torch.load(WORK_PATH+'0830.{}.MY_chem_B_feat.pt'.format(G_NAME))
MY_chem_A_adj = torch.load(WORK_PATH+'0830.{}.MY_chem_A_adj.pt'.format(G_NAME))
MY_chem_B_adj = torch.load(WORK_PATH+'0830.{}.MY_chem_B_adj.pt'.format(G_NAME))
MY_exp_A = torch.load(WORK_PATH+'0830.{}.MY_exp_A.pt'.format(G_NAME))
MY_exp_B = torch.load(WORK_PATH+'0830.{}.MY_exp_B.pt'.format(G_NAME))
MY_exp_AB = torch.load(WORK_PATH+'0830.{}.MY_exp_AB.pt'.format(G_NAME))
MY_Cell = torch.load(WORK_PATH+'0830.{}.MY_Cell.pt'.format(G_NAME))
MY_tgt_A = torch.load(WORK_PATH+'0830.{}.MY_tgt_A.pt'.format(G_NAME))
MY_tgt_B = torch.load(WORK_PATH+'0830.{}.MY_tgt_B.pt'.format(G_NAME))
MY_syn = torch.load(WORK_PATH+'0830.{}.MY_syn.pt'.format(G_NAME))
...
PRJ_PATH = '/st06/jiyeonH/11.TOX/MY_TRIAL_GPU/PJ01.TRIAL.3_9_1/'
ANA_DF = pd.read_csv(PRJ_PATH+'RAY_ANA_DF.P01.3_9_1.csv')
with open(PRJ_PATH+'RAY_ANA_DF.P01.3_9_1.pickle', 'rb') as f:
...
PRJ_PATH, 'TRIAL.3_9_1.{}.BEST.loss'.format(G_NAME) )
...
plot_loss(list(mini_df.TrainLoss), list(mini_df.ValLoss), 
PRJ_PATH, 'TRIAL.3_9_1.{}.MIN.loss'.format(G_NAME) )
