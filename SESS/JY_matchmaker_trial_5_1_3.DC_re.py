...
WORK_PATH = '/home01/k006a01/PRJ.01/TRIAL_5.1/'
...
BETA_CP_info = pd.read_table('/st06/jiyeonH/13.DD_SESS/LINCS_BETA/'+'compoundinfo_beta.txt') # 39321
...
# pert filter
filter1 = BETA_SIG_info[BETA_SIG_info.pert_type.isin(['ctl_vehicle', 'ctl_untrt' ,'trt_cp' ])] # 764996

# exempler filter 
filter2 = filter1[filter1.is_exemplar_sig==1] # 136460

BETA_MJ = pd.read_csv('/st06/jiyeonH/13.DD_SESS/LINCS_BETA/'+"Lincs_pubchem_mj.csv") # SID 필터까지 그렇게 완성한 무엇  # 127595
...
BETA_MJ_RE = BETA_MJ[['pert_id','SMILES_cid','canonical_smiles',
	   ...
	   'canonical_smiles_re', 'tpsa']].drop_duplicates() # 25903
...
DC_DATA7_1['drug_row_cid_re'] = [int(a) if type(a) == int else 0 for a in list(DC_DATA7_1['drug_row_cid'])] # {<class 'NoneType'>, <class 'int'>}
DC_DATA7_1['drug_col_cid_re'] = [int(a) if type(a) == int else 0 for a in list(DC_DATA7_1['drug_col_cid'])] # {<class 'NoneType'>, <class 'int'>}
...
# DC set 확인 
A_B_C_S = DATA_AO_BO.reset_index() # 11379
...
A_B_C_S_SET = A_B_C_S_SET.reset_index(drop=True) # 8914
...
original_cids = copy.deepcopy(DATA_AO_BO_cids) ############################

DATA_Filter_1 = BETA_EXM2[['pubchem_cid','canonical_smiles_re']].drop_duplicates()

cids_all = list(set(list(A_B_C_S_SET.drug_row_cid) + list(A_B_C_S_SET.drug_col_cid))) # 172

DATA_Filter_2 = DATA_Filter_1[DATA_Filter_1.pubchem_cid.isin(cids_all)]
DATA_Filter_ori = DATA_Filter_1[DATA_Filter_1.pubchem_cid.isin(original_cids)]
...
means_df['cat']=['MEAN' for a in range(len(original_cids))]
...
A_B_C_S_SET = copy.deepcopy(tmp_col) # 7720
...
# sms_all = list(set(list(A_B_C_S_SET.drug_row_sm) + list(A_B_C_S_SET.drug_col_sm))) # 164 
...
torch.save(MY_chem_A_feat, WORK_PATH+'0928.{}.MY_chem_A_feat.pt'.format(G_NAME))
torch.save(MY_chem_B_feat, WORK_PATH+'0928.{}.MY_chem_B_feat.pt'.format(G_NAME))
torch.save(MY_chem_A_adj, WORK_PATH+'0928.{}.MY_chem_A_adj.pt'.format(G_NAME))
torch.save(MY_chem_B_adj, WORK_PATH+'0928.{}.MY_chem_B_adj.pt'.format(G_NAME))
torch.save(MY_exp_A, WORK_PATH+'0928.{}.MY_exp_A.pt'.format(G_NAME))
torch.save(MY_exp_B, WORK_PATH+'0928.{}.MY_exp_B.pt'.format(G_NAME))
torch.save(MY_exp_AB, WORK_PATH+'0928.{}.MY_exp_AB.pt'.format(G_NAME))
torch.save(MY_Cell, WORK_PATH+'0928.{}.MY_Cell.pt'.format(G_NAME))
torch.save(MY_tgt_A, WORK_PATH+'0928.{}.MY_tgt_A.pt'.format(G_NAME))
torch.save(MY_tgt_B, WORK_PATH+'0928.{}.MY_tgt_B.pt'.format(G_NAME))
torch.save(MY_syn, WORK_PATH+'0928.{}.MY_syn.pt'.format(G_NAME))
...
def RAY_MY_train(config, checkpoint_dir=None):
	...
	print("Finished Training")
															...
															loaders = {
																		'train' : torch.utils.data.DataLoader(T_train, batch_size = 16, collate_fn = graph_collate_fn, shuffle =False, num_workers=1),
															}
...
# local
MAIN('22.09.28.PRJ01.TRIAL5_1', WORK_PATH, WORK_PATH, '5_1', 'DC_filt', 3, 3, 1, 32, 1)

# 4gpu
MAIN('22.09.28.PRJ01.TRIAL5_1', WORK_PATH, WORK_PATH, '5_1', 'DC_filt', 100, 1000, 150, 32, 1)

# 8gpu
MAIN('22.09.28.PRJ01.TRIAL5_1', WORK_PATH, WORK_PATH, '5_1', 'DC_filt', 100, 1000, 150, 16, 1)
MAIN('22.09.28.PRJ01.TRIAL5_1', WORK_PATH, WORK_PATH, '5_1', 'DC_filt', 100, 1000, 150, 8, 0.5)
...
RAY_dir = '22.09.28.PRJ01.TRIAL5_1'
G_NAME = 'DC_filt' 
K_W_Dir = 'TRIAL_5.1' #  TRIAL_5.1 , TRIAL.5_2_3 , TRIAL.5_3_3
...
DF_KEY
# get /home01/k006a01/ray_results/22.09.28.PRJ01.TRIAL5_1/RAY_MY_train_7ce0c57c_62_G_hiddim=64,G_layer=3,batch_size=16,dropout_1=0.2000,dropout_2=0.2000,epoch=1000,feat_size_0=128,feat_siz_2022-09-30_13-28-06/model.pth M1_model.pth
...
# get /home01/k006a01/ray_results/22.09.28.PRJ01.TRIAL5_1/RAY_MY_train_7ce0c57c_62_G_hiddim=64,G_layer=3,batch_size=16,dropout_1=0.2000,dropout_2=0.2000,epoch=1000,feat_size_0=128,feat_siz_2022-09-30_13-28-06/checkpoint_000816/checkpoint M2_checkpoint
...
# 130
# get /home01/k006a01/ray_results/22.09.28.PRJ01.TRIAL5_1/RAY_MY_train_f2798f4c_43_G_hiddim=64,G_layer=2,batch_size=16,dropout_1=0.2000,dropout_2=0.2000,epoch=1000,feat_size_0=64,feat_size_2022-09-29_18-08-07/checkpoint_000808/checkpoint M4_checkpoint
...
WORK_PATH = '/st06/jiyeonH/11.TOX/MY_TRIAL_GPU/PJ01.TRIAL.5_1_3/'

G_NAME='DC_filt'

MY_chem_A_feat = torch.load(WORK_PATH+'0928.{}.MY_chem_A_feat.pt'.format(G_NAME))
MY_chem_B_feat = torch.load(WORK_PATH+'0928.{}.MY_chem_B_feat.pt'.format(G_NAME))
MY_chem_A_adj = torch.load(WORK_PATH+'0928.{}.MY_chem_A_adj.pt'.format(G_NAME))
MY_chem_B_adj = torch.load(WORK_PATH+'0928.{}.MY_chem_B_adj.pt'.format(G_NAME))
MY_exp_A = torch.load(WORK_PATH+'0928.{}.MY_exp_A.pt'.format(G_NAME))
MY_exp_B = torch.load(WORK_PATH+'0928.{}.MY_exp_B.pt'.format(G_NAME))
MY_exp_AB = torch.load(WORK_PATH+'0928.{}.MY_exp_AB.pt'.format(G_NAME))
MY_Cell = torch.load(WORK_PATH+'0928.{}.MY_Cell.pt'.format(G_NAME))
MY_tgt_A = torch.load(WORK_PATH+'0928.{}.MY_tgt_A.pt'.format(G_NAME))
MY_tgt_B = torch.load(WORK_PATH+'0928.{}.MY_tgt_B.pt'.format(G_NAME))
MY_syn = torch.load(WORK_PATH+'0928.{}.MY_syn.pt'.format(G_NAME))
...
A_B_C_S_SM1 = pd.merge(A_B_C_S_SET_COH, for_CAN_smiles, on='drug_row_cid', how ='left' )
...
chem_feat_A_train, chem_feat_A_tv, chem_feat_B_train, chem_feat_B_tv, chem_adj_A_train, chem_adj_A_tv, chem_adj_B_train, chem_adj_B_tv, exp_A_train, exp_A_tv, exp_B_train, exp_B_tv, tgt_A_train, tgt_A_tv, tgt_B_train, tgt_B_tv, syn_train, syn_tv, cell_train, cell_tv, SM_t, SM_tv = sklearn.model_selection.train_test_split(
		MY_chem_A_feat, MY_chem_B_feat, MY_chem_A_adj, MY_chem_B_adj, MY_exp_A, MY_exp_B, MY_tgt_A, MY_tgt_B, MY_syn, MY_Cell, A_B_C_S_SM3,
		test_size= 0.2 , random_state=42 )
chem_feat_A_val, chem_feat_A_test, chem_feat_B_val, chem_feat_B_test, chem_adj_A_val, chem_adj_A_test, chem_adj_B_val, chem_adj_B_test, exp_A_val, exp_A_test, exp_B_val, exp_B_test, tgt_A_val, tgt_A_test, tgt_B_val, tgt_B_test, syn_val, syn_test, cell_val, cell_test, SM_v, SM_test  = sklearn.model_selection.train_test_split(
		chem_feat_A_tv, chem_feat_B_tv, chem_adj_A_tv, chem_adj_B_tv, exp_A_tv, exp_B_tv, tgt_A_tv, tgt_B_tv, syn_tv, cell_tv, SM_tv,
		test_size=0.5, random_state=42 )
...
T_test = DATASET_GCN_W_FT(
	...
	test_data['TGT_A'][selec_ind], test_data['TGT_B'][selec_ind], 
	JY_ADJ_IDX, JY_IDX_WEIGHT_T, 
...
PRJ_PATH = '/st06/jiyeonH/11.TOX/MY_TRIAL_GPU/PJ01.TRIAL.5_1_3/'
ANA_DF = pd.read_csv(PRJ_PATH+'RAY_ANA_DF.5_1_3.csv')
with open(PRJ_PATH+'RAY_ANA_DF.5_1_3.pickle', 'rb') as f:
...
device = "cuda:0" if torch.cuda.is_available() else "cpu"
if torch.cuda.is_available():
	state_dict = torch.load(os.path.join(PRJ_PATH, 'M1_model.pth'))
else:
...
TEST_LOSS = test_loss/(batch_idx_t+1)
R__T = TEST_LOSS
R__1 , R__2 = jy_corrplot(PRED_list, Y_list, PRJ_PATH,'P.{}.{}.{}_model_retest'.format(Trial_name, G_NAME, 'M1') )
