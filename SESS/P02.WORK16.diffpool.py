...
WORK_NAME = 'WORK_16' # full / new target / cut1 / diff pool
...
class DATASET_DIFF_W_FT(Dataset):
	...
	def __getitem__(self, index):
		...
		#
		return self.gcn_drug1_F[index], self.gcn_drug2_F[index], self.gcn_drug1_ADJ[index], self.gcn_drug2_ADJ[index], FEAT_A, FEAT_B, self.gcn_adj, self.gcn_adj_weight , self.cell_info[index], self.syn_ans[index]



							gcn_drug1_F___1 = MY_chem_A_feat_RE2[0]
							gcn_drug1_F___2 = MY_chem_A_feat_RE2[1]

							gcn_drug2_F___1 = MY_chem_B_feat_RE2[0]
							gcn_drug2_F___2 = MY_chem_B_feat_RE2[1]

							gcn_drug1_ADJ___1 = MY_chem_A_adj_RE2[0]
							gcn_drug1_ADJ___2 = MY_chem_A_adj_RE2[1]

							gcn_drug2_ADJ___1 = MY_chem_B_adj_RE2[0]
							gcn_drug2_ADJ___2 = MY_chem_B_adj_RE2[1]

							gcn_gene_A___1 = MY_g_EXP_A_RE2[0]
							gcn_gene_A___2 = MY_g_EXP_A_RE2[1]

							gcn_gene_B___1 = MY_g_EXP_B_RE2[0]
							gcn_gene_B___2 = MY_g_EXP_B_RE2[1]

							target_A___1 = MY_Target_A2[0]
							target_A___2 = MY_Target_A2[1]

							target_B___1 = MY_Target_B2[0]
							target_B___2 = MY_Target_B2[1]

							cell_basal___1 = MY_CellBase_RE2[0]
							cell_basal___2 = MY_CellBase_RE2[1]

							FEAT___1 = torch.Tensor(np.array([ gcn_gene_A___1.squeeze().tolist() , target_A___1.tolist(), cell_basal___1.tolist()]).T)
							gcn_adj___1 = JY_ADJ_tmp
							gcn_adj___2 = JY_ADJ_tmp

							gcn_adj_weight___1 = JY_IDX_WEIGHT_T
							gcn_adj_weight___2 = JY_IDX_WEIGHT_T

							syn_ans___1 = MY_syn_RE2[0]
							syn_ans___2 = MY_syn_RE2[1]

							cell_info___1 = cell_one_hot[0]
							cell_info___2 = cell_one_hot[1]
...
RAY_loss_weight = ray.put(loss_weight)
					...
					RAY_val = ray.put(T_val)
					RAY_test = ray.put(T_test)
					RAY_loss_weight = ray.put(loss_weight[0:128])

					for i,a in enumerate(T_train):
						a

					drug1_f, drug2_f, drug1_a, drug2_a, expA, expB, adj, adj_w, cell, y = a
...
# 이번엔 diff pool 뺐는데,(나름의 normalization 의 일환으로 원래는 넣었지만.. )
# 결과 별로면 다시 넣어보는걸로... 
class DIFFPOOL(torch.nn.Module):
	...
	def forward(self, x, adj, mask = None):
		...
		return x
					...
					# config["G_pool_chem"], config["G_pool_exp"], config["G_ratio_chem"], config["G_ratio_exp"], 
...
def RAY_MY_train(config, checkpoint_dir=None):
	...
	for epoch in range(n_epochs):
		...
		for batch_idx_t, (drug1_f, drug2_f, drug1_a, drug2_a, expA, expB, adj, adj_w, cell, y) in enumerate(loaders['train']):
			...
			optimizer.zero_grad()
			output = MM_MODEL(drug1_f, drug2_f, drug1_a, drug2_a, expA, expB, adj.float(), adj_w, cell, y)
...
def RAY_TEST_MODEL(my_config, model_path, model_name, PRJ_PATH, PRJ_NAME, MISS_NAME, number): 
	...
	with torch.no_grad():
		...
		for batch_idx_t, (drug1_f, drug2_f, drug1_a, drug2_a, expA, expB, adj, adj_w, cell, y) in enumerate(Test_loader):
			...
			if use_cuda:
				drug1_f, drug2_f, drug1_a, drug2_a, expA, expB, adj, adj_w, y, cell = drug1_f.cuda(), drug2_f.cuda(), drug1_a.cuda(), drug2_a.cuda(), expA.cuda(), expB.cuda(), adj.cuda(), adj_w.cuda(), y.cuda(), cell.cuda()
			output= best_model(drug1_f, drug2_f, drug1_a, drug2_a, expA, expB, adj.float(), adj_w, cell, y) 
			MSE = torch.nn.MSELoss()
			loss = MSE(output, y)
...
def MAIN(ANAL_name, WORK_PATH, PRJ_PATH, PRJ_NAME, MISS_NAME, num_samples= 10, max_num_epochs=1000, grace_period = 150, cpus_per_trial = 16, gpus_per_trial = 1):
	...
	CONFIG={
		...
		"G_pool_chem" : tune.choice([2, 3, 4]), #		
		"G_pool_exp" : tune.choice([2, 3, 4]), #		
		"G_ratio_chem" : tune.choice([0.5, 0.8]),
		"G_ratio_exp" : tune.choice([0.2, 0.5, 0.8]),
