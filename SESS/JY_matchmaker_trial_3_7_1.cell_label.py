...
C_df = pd.merge(C_df,DC_CELL_DF3[['DC_cellname','tissue']], left_on ='cell', right_on='DC_cellname', how='left' )
fig = plt.figure(figsize=(10,8))
plt.bar(C_df['cell'], C_df['freq'], color=C_df['tissue'])
plt.xticks(range(C_df.shape[0]),list(C_df['cell']), rotation=90)
for i in range(C_df.shape[0]):
	plt.annotate(str(list(C_df['freq'])[i]), xy=(i,list(C_df['freq'])[i]), ha='center', va='bottom')
...
MY_Cell = torch.empty(size=(10, cell_one_hot.shape[1]))
...
torch.save(MY_chem_A_feat, WORK_PATH+'0824.{}.MY_chem_A_feat.pt'.format(G_NAME))
torch.save(MY_chem_B_feat, WORK_PATH+'0824.{}.MY_chem_B_feat.pt'.format(G_NAME))
torch.save(MY_chem_A_adj, WORK_PATH+'0824.{}.MY_chem_A_adj.pt'.format(G_NAME))
torch.save(MY_chem_B_adj, WORK_PATH+'0824.{}.MY_chem_B_adj.pt'.format(G_NAME))
torch.save(MY_exp_A, WORK_PATH+'0824.{}.MY_exp_A.pt'.format(G_NAME))
torch.save(MY_exp_B, WORK_PATH+'0824.{}.MY_exp_B.pt'.format(G_NAME))
torch.save(MY_exp_AB, WORK_PATH+'0824.{}.MY_exp_AB.pt'.format(G_NAME))
torch.save(MY_Cell, WORK_PATH+'0824.{}.MY_Cell.pt'.format(G_NAME))
torch.save(MY_tgt_A, WORK_PATH+'0824.{}.MY_tgt_A.pt'.format(G_NAME))
torch.save(MY_tgt_B, WORK_PATH+'0824.{}.MY_tgt_B.pt'.format(G_NAME))
torch.save(MY_syn, WORK_PATH+'0824.{}.MY_syn.pt'.format(G_NAME))
...
def RAY_TEST_MODEL(best_trial): 
	...
	for batch_idx_t, (drug1_f, drug2_f, drug1_a, drug2_a, expA, expB, adj, adj_w, y, cell) in enumerate(Test_loader):
		...
		if use_cuda:
			drug1_f, drug2_f, drug1_a, drug2_a, expA, expB, adj, adj_w, y, cell = drug1_f.cuda(), drug2_f.cuda(), drug1_a.cuda(), drug2_a.cuda(), expA.cuda(), expB.cuda(), adj.cuda(), adj_w.cuda(), y.cuda(), cell.cuda()
		output = best_trained_model(drug1_f, drug2_f, drug1_a, drug2_a, expA, expB, adj, adj_w, y, cell) 
		MSE = torch.nn.MSELoss()
		loss = MSE(output, y)
...
# MAIN('22.08.24.PRJ01.TRIAL3_7_1', WORK_PATH, 6, 1000, 600, 32, 1)
# MAIN('22.08.24.PRJ01.TRIAL3_7_1', WORK_PATH, 3, 5, 2, 8, 1)
MAIN('22.08.24.PRJ01.TRIAL3_7_1', WORK_PATH, 100, 1000, 150, 32, 1)
...
expA = expA.view(-1,2)#### 다른점 
expB = expB.view(-1,2)#### 다른점 
MM_MODEL(drug1_f, drug2_f, drug1_a, drug2_a, expA, expB, adj, adj_w, y, cell)
...
anal_df = ExperimentAnalysis("~/ray_results/22.08.24.PRJ01.TRIAL3_7_1")


ANA_DF = anal_df.dataframe()
ANA_ALL_DF = anal_df.trial_dataframes

ANA_DF.to_csv('/home01/k006a01/PRJ.01/TRIAL_3.7/RAY_ANA_DF.P01.3_7_1.csv')
import pickle
with open("/home01/k006a01/PRJ.01/TRIAL_3.7/RAY_ANA_DF.P01.3_7_1.pickle", "wb") as fp:
	pickle.dump(ANA_ALL_DF,fp) 


DF_KEY = list(ANA_DF.sort_values('ValLoss')['logdir'])[0]
DF_KEY
# get /home01/k006a01/ray_results/22.08.24.PRJ01.TRIAL3_7_1/RAY_MY_train_0336dbe8_76_G_hiddim=128,G_layer=2,batch_size=64,dropout_1=0.0100,dropout_2=0.0100,epoch=1000,feat_size_0=256,feat_si_2022-08-25_19-49-59/model.pth M1_model.pth
...
TOPVAL_PATH
# get /home01/k006a01/ray_results/22.08.24.PRJ01.TRIAL3_7_1/RAY_MY_train_0336dbe8_76_G_hiddim=128,G_layer=2,batch_size=64,dropout_1=0.0100,dropout_2=0.0100,epoch=1000,feat_size_0=256,feat_si_2022-08-25_19-49-59/checkpoint_000986/checkpoint M2_checkpoint
...
# get /home01/k006a01/ray_results/22.08.24.PRJ01.TRIAL3_7_1/RAY_MY_train_879d3d54_97_G_hiddim=128,G_layer=2,batch_size=64,dropout_1=0.0100,dropout_2=0.0100,epoch=1000,feat_size_0=256,feat_si_2022-08-26_08-57-40/checkpoint_000753/checkpoint M4_checkpoint
...
결과 확인 
WORK_PATH = '/st06/jiyeonH/11.TOX/MY_TRIAL_GPU/PJ01.TRIAL.3_7_1/'


G_NAME = 'Cell_Add'
MY_chem_A_feat = torch.load(WORK_PATH+'0824.{}.MY_chem_A_feat.pt'.format(G_NAME))
MY_chem_B_feat = torch.load(WORK_PATH+'0824.{}.MY_chem_B_feat.pt'.format(G_NAME))
MY_chem_A_adj = torch.load(WORK_PATH+'0824.{}.MY_chem_A_adj.pt'.format(G_NAME))
MY_chem_B_adj = torch.load(WORK_PATH+'0824.{}.MY_chem_B_adj.pt'.format(G_NAME))
MY_exp_A = torch.load(WORK_PATH+'0824.{}.MY_exp_A.pt'.format(G_NAME))
MY_exp_B = torch.load(WORK_PATH+'0824.{}.MY_exp_B.pt'.format(G_NAME))
MY_exp_AB = torch.load(WORK_PATH+'0824.{}.MY_exp_AB.pt'.format(G_NAME))
MY_Cell = torch.load(WORK_PATH+'0824.{}.MY_Cell.pt'.format(G_NAME))
MY_tgt_A = torch.load(WORK_PATH+'0824.{}.MY_tgt_A.pt'.format(G_NAME))
MY_tgt_B = torch.load(WORK_PATH+'0824.{}.MY_tgt_B.pt'.format(G_NAME))
MY_syn = torch.load(WORK_PATH+'0824.{}.MY_syn.pt'.format(G_NAME))
...
PRJ_PATH = '/st06/jiyeonH/11.TOX/MY_TRIAL_GPU/PJ01.TRIAL.3_7_1/'
ANA_DF = pd.read_csv(PRJ_PATH+'RAY_ANA_DF.P01.3_7_1.csv')
with open(PRJ_PATH+'RAY_ANA_DF.P01.3_7_1.pickle', 'rb') as f:
...
PRJ_PATH, 'TRIAL.3_7_1.{}.BEST.loss'.format(G_NAME) )
...
R_1_T = TEST_LOSS
R_1_1 , R_1_2 = jy_corrplot(PRED_list, Y_list, PRJ_PATH,'P.3_7_1.{}.M1_model'.format(G_NAME) )
...
R_2_T = TEST_LOSS
R_2_1 , R_2_2 = jy_corrplot(PRED_list, Y_list,PRJ_PATH,'P.3_7_1.{}.M2_checkpoint'.format(G_NAME) )
...
mini_df = ANA_ALL_DF[TOT_key]
plot_loss(list(mini_df.TrainLoss), list(mini_df.ValLoss), 
PRJ_PATH, 'TRIAL.3_7_1.{}.MIN.loss'.format(G_NAME) )
...
R_3_T = TEST_LOSS
R_3_1 , R_3_2 = jy_corrplot(PRED_list, Y_list,PRJ_PATH,'P.3_7_1.{}.M4_checkpoint'.format(G_NAME) )
