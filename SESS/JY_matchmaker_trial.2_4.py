...
#MY_tgt_A_tch = torch.tensor(np.array(MY_tgt_A))
#MY_tgt_B_tch = torch.tensor(np.array(MY_tgt_B))
#MY_syn_tch = torch.tensor(np.array(MY_syn))


#torch.save(MY_chem_A, WORK_PATH+'0627.MY_chem_A.pt')
#torch.save(MY_chem_B, WORK_PATH+'0627.MY_chem_B.pt')
#torch.save(MY_exp_A, WORK_PATH+'0627.MY_exp_A.pt')
#torch.save(MY_exp_B, WORK_PATH+'0627.MY_exp_B.pt')
#torch.save(MY_exp_AB, WORK_PATH+'0627.MY_exp_AB.pt')
#torch.save(MY_tgt_A, WORK_PATH+'0627.MY_tgt_A.pt')
...
MY_exp_B_tch = torch.load(Tch_PATH+'0627.MY_exp_B.pt')
MY_exp_AB_tch = torch.load(Tch_PATH+'0627.MY_exp_AB.pt')
MY_tgt_A_tch = torch.load(Tch_PATH+'0627.MY_tgt_A.pt')
MY_tgt_B_tch = torch.load(Tch_PATH+'0627.MY_tgt_B.pt')
MY_syn_tch = torch.load(Tch_PATH+'0627.MY_syn.pt')

#MY_chem_A_tch = torch.load(PRJ_PATH+'0627.MY_chem_A.pt')
#MY_chem_B_tch = torch.load(PRJ_PATH+'0627.MY_chem_B.pt')
#MY_exp_A_tch = torch.load(PRJ_PATH+'0627.MY_exp_A.pt')
#MY_exp_B_tch = torch.load(PRJ_PATH+'0627.MY_exp_B.pt')
#MY_exp_AB_tch = torch.load(PRJ_PATH+'0627.MY_exp_AB.pt')
#MY_tgt_A_tch = torch.load(PRJ_PATH+'0627.MY_tgt_A.pt')
#MY_tgt_B_tch = torch.load(PRJ_PATH+'0627.MY_tgt_B.pt')
#MY_syn_tch = torch.load(PRJ_PATH+'0627.MY_syn.pt')
...
def prepare_data_GCN(MY_chem_A_tch, MY_chem_B_tch, MY_exp_A_tch, MY_exp_B_tch, MY_tgt_A_tch, MY_tgt_B_tch, MY_syn_tch, norm ) :
	chem_A_train, chem_A_tv, chem_B_train, chem_B_tv, exp_A_train, exp_A_tv, exp_B_train, exp_B_tv, tgt_A_train, tgt_A_tv, tgt_B_train, tgt_B_tv, syn_train, syn_tv = sklearn.model_selection.train_test_split(
			MY_chem_A_tch, MY_chem_B_tch, MY_exp_A_tch, MY_exp_B_tch, MY_tgt_A_tch, MY_tgt_B_tch, MY_syn_tch,
			test_size= 0.2 , random_state=42 )
	chem_A_val, chem_A_test, chem_B_val, chem_B_test, exp_A_val, exp_A_test, exp_B_val, exp_B_test, tgt_A_val, tgt_A_test, tgt_B_val, tgt_B_test, syn_val, syn_test  = sklearn.model_selection.train_test_split(
			chem_A_tv, chem_B_tv, exp_A_tv, exp_B_tv, tgt_A_tv, tgt_B_tv, syn_tv,
...
class DATASET_GCN_W_FT(Dataset):
	def __init__(self, drug1_F, drug2_F, gcn_exp_A, gcn_exp_B, gcn_tgt_A, gcn_tgt_B, gcn_adj, gcn_adj_weight, syn_ans):
	...
	def __getitem__(self, index):
		...
		return self.drug1_F[index], self.drug2_F[index], FEAT_A, FEAT_B, self.gcn_adj, self.gcn_adj_weight, self.syn_ans[index]
...
norm = 'tanh_norm'
train_data, val_data, test_data = prepare_data_GCN(MY_chem_A_tch, MY_chem_B_tch, MY_exp_A_tch, MY_exp_B_tch, MY_tgt_A_tch, MY_tgt_B_tch, MY_syn_tch, norm)
...
# DATA check 
T_train = DATASET_GCN_W_FT(torch.Tensor(train_data['drug1']), torch.Tensor(train_data['drug2']), train_data['EXP_A'], train_data['EXP_B'], train_data['TGT_A'], train_data['TGT_B'], JY_ADJ_IDX, JY_IDX_WEIGHT_T, torch.Tensor(train_data['y']))
T_val = DATASET_GCN_W_FT(torch.Tensor(val_data['drug1']), torch.Tensor(val_data['drug2']), val_data['EXP_A'], val_data['EXP_B'], val_data['TGT_A'], val_data['TGT_B'], JY_ADJ_IDX, JY_IDX_WEIGHT_T, torch.Tensor(val_data['y']))
T_test = DATASET_GCN_W_FT(torch.Tensor(test_data['drug1']), torch.Tensor(test_data['drug2']), test_data['EXP_A'], test_data['EXP_B'], test_data['TGT_A'], test_data['TGT_B'], JY_ADJ_IDX, JY_IDX_WEIGHT_T, torch.Tensor(test_data['y']))
...
def RAY_MY_train(config, checkpoint_dir=None):
	...
	MM_MODEL = MY_expGCN_parallel_model(
			...
			T_train.drug1_F.shape[1], T_train.drug1_F.shape[1],
...
def RAY_TEST_MODEL(best_trial): 
	...
	best_trained_model = MY_expGCN_parallel_model(
		...
		T_test.drug1_F.shape[1], T_test.drug1_F.shape[1],
		dsn1_layers, dsn2_layers, snp_layers, 1,
		inDrop, Drop
	)
...
MAIN(WORK_PATH, 100, 1000, 150, 32, 1)
...
# G_layer, G_indim, G_hiddim, drug1_indim, drug2_indim, layers_1, layers_2, layers_3, out_dim, inDrop, drop
...
loaders = {
	'train' : torch.utils.data.DataLoader(T_train, batch_size = 10, collate_fn = graph_collate_fn, shuffle =False), # , num_workers=1
}

DLDL = torch.utils.data.DataLoader(T_train, batch_size = 10, collate_fn = graph_collate_fn, shuffle =False)
...
#############
결과 확인  (6/30 기준 아직 돌아가는중이지만! )
#############

from ray.tune import ExperimentAnalysis
# anal_df = ExperimentAnalysis("~/ray_results/22.06.27.PRJ01.TRIAL2_4")

ANA_DF = anal_df.dataframe()
ANA_ALL_DF = anal_df.trial_dataframes

#ANA_DF.to_csv('/home01/k006a01/PRJ.01/TRIAL_2.4/RAY_ANA_DF.P01.2_4.csv')
#import pickle
#with open("/home01/k006a01/PRJ.01/TRIAL_2.4/RAY_ANA_ALL_DF.P01.2_4.pickle", "wb") as fp:
#	pickle.dump(ANA_ALL_DF,fp) 

PRJ_PATH = '/st06/jiyeonH/11.TOX/MY_TRIAL_GPU/PJ01.TRIAL.2_4/'
#ANA_DF = pd.read_csv(PRJ_PATH+'RAY_ANA_DF.P01.2_4.csv')
#with open(PRJ_PATH+'RAY_ANA_ALL_DF.P01.2_4.pickle', 'rb') as f:
...
# /home01/k006a01/ray_results/22.06.27.PRJ01.TRIAL2_4/RAY_MY_train_20d79d42_15_G_hiddim=512,G_layer=4,batch_size=64,dropout_1=0.5000,dropout_2=0.2000,epoch=1000,feat_size_0=1024,feat_s_2022-06-28_20-34-54/checkpoint_000241
min(mini_df.ValLoss)
...
TOT_min
# /home01/k006a01/ray_results/22.06.27.PRJ01.TRIAL2_4/RAY_MY_train_24f5e71e_19_G_hiddim=32,G_layer=4,batch_size=64,dropout_1=0.5000,dropout_2=0.2000,epoch=1000,feat_size_0=1024,feat_si_2022-06-29_05-53-08


mini_df = ANA_ALL_DF[TOT_key]
plot_loss(list(mini_df.TrainLoss), list(mini_df.ValLoss), 
PRJ_PATH, 'TRIAL_2_4.MIN.loss' )
...
# /home01/k006a01/ray_results/22.06.27.PRJ01.TRIAL2_4/RAY_MY_train_24f5e71e_19_G_hiddim=32,G_layer=4,batch_size=64,dropout_1=0.5000,dropout_2=0.2000,epoch=1000,feat_size_0=1024,feat_si_2022-06-29_05-53-08/checkpoint_000241
