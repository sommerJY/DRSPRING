...
CELLO_TABLE = CELLO_TABLE = pd.read_csv(MM_PATH+'cellosaurus.txt', skiprows = 54, na_values='NaN', prefix='V', header = None, sep = '   ' )
...
def prepare_data(MY_chem1_tch, MY_chem2_tch, MY_exp_A_tch, MY_exp_B_tch, MY_syn_tch, norm ) :
	...
	#
	train_data['drug1'] = np.concatenate((train_data['drug1'],train_data['exp1']),axis=1)
	train_data['drug2'] = np.concatenate((train_data['drug2'],train_data['exp2']),axis=1)
	#
	val_data['drug1'] = np.concatenate((val_data['drug1'],val_data['exp1']),axis=1)
	val_data['drug2'] = np.concatenate((val_data['drug2'],val_data['exp2']),axis=1)
	#
	test_data['drug1'] = np.concatenate((test_data['drug1'],test_data['exp1']),axis=1)
	test_data['drug2'] = np.concatenate((test_data['drug2'],test_data['exp2']),axis=1)
	#		
...
train_data, val_data, test_data = prepare_data(
	MY_chem_A_tch, MY_chem_B_tch, MY_exp_A_tch, MY_exp_B_tch, MY_syn_tch, norm)
...
def MAIN(num_samples= 10, max_num_epochs=1000, grace_period = 200, cpus_per_trial = 16, gpus_per_trial = 1):
	...
	plot_loss(list(TMP_DF.TrainLoss), list(TMP_DF.ValLoss), '/st06/jiyeonH/11.TOX/MY_TRIAL_5/01.Trial_Matchmaker/my_trial_1_2/', 'MM_GCNexp_IDK')
	# 
...
CONFIG={
		'n_workers' : tune.choice([8]),
		"epoch" : tune.choice([1000]),
		"batch_size" : tune.choice([128, 64, 32]), # The number of batch sizes should be a power of 2 to take full advantage of the GPUs processing (근데 256 은 큰가봉가. 메모리 에러 남)
		"feat_size_0" : tune.choice([4096, 2048, 1024, 512, 256, 128, 64, 32]),
		"feat_size_1" : tune.choice([4096, 2048, 1024, 512, 256, 128, 64, 32]),
		"feat_size_2" : tune.choice([4096, 2048, 1024, 512, 256, 128, 64, 32]),
...
#
ASHA_scheduler = tune.schedulers.ASHAScheduler(
	time_attr='training_iteration', metric="ValLoss", mode="min", max_t= 1000, grace_period = 100)


ANALYSIS = tune.run( # 끝내지 않음 
		tune.with_parameters(RAY_MY_train),  
		name = '22.06.15.PRJ01.TRIAL.1_2_re', # 22.06.15.PRJ01.TRIAL.1_2 가 원래인데 메모리 에러 ㅅㅂ 
		num_samples=100, 
		config=CONFIG, 
...
plot_loss(list(mini_df.TrainLoss), list(mini_df.ValLoss), '/st06/jiyeonH/11.TOX/MY_TRIAL_5/01.Trial_Matchmaker/my_trial_1_2', 'TRIAL_1_2.num99.loss' )





dsn1_layers = [64, 4096, 256]
dsn2_layers = [64, 4096, 256] 
snp_layers = [4096 , 128]
inDrop = 0.2
Drop = 0.01
...
T_test = ray.get(RAY_test)
Test_loader = torch.utils.data.DataLoader(T_test, batch_size = 32, shuffle =False)
