...
#DATA_PATH = '/st06/jiyeonH/11.TOX/DR_SPRING/trials/M1_J1/'


NETWORK_PATH = '/home01/k020a01/01.Data/HumanNet/'
DATA_PATH = '/home01/k020a01/02.Trial/M1_J1_DATA/'
...
# SAVE_PATH = '/st06/jiyeonH/11.TOX/DR_SPRING/trials/{}/{}_DATA/'.format(PRJ_NAME,PRJ_NAME)
...
node_names = [a.split('__')[1] for a in ID_G_RE.nodes()]

row_targets = []
col_targets = []

아예 서버에서 노드 바꿔서 가져가는거 짜고 있었음 
...
A_B_C_S_SET_SM = copy.deepcopy(A_B_C_S_SET_COH)
...
def RAY_MY_train(config, checkpoint_dir=None):
	...
	MM_MODEL = MY_expGCN_parallel_model(
			...
			dsn1_layers, dsn2_layers, snp_layers, len(set(A_B_C_S_SET_SM.DrugCombCello)), 1,
			inDrop, Drop
			)
	#
	if torch.cuda.is_available():
...
def MAIN(ANAL_name, WORK_PATH, PRJ_PATH, Trial_name, G_NAME, num_samples= 10, max_num_epochs=1000, grace_period = 150, cpus_per_trial = 16, gpus_per_trial = 1):
	...
	CONFIG={
		...
		"G_hiddim" : tune.choice([256, 128, 64, 32, 16, 8]), # 
	...
	ANALYSIS = tune.run( # 끝내지 않음 
		tune.with_parameters(RAY_MY_train, config=CONFIG, ),
		name = ANAL_name,
...
# cpu test 
MAIN('{}.PRJ02.{}.{}'.format(WORK_DATE,PRJ_NAME, MISS_NAME), WORK_PATH, WORK_PATH, PRJ_NAME, MISS_NAME, 2, 2, 1, 16, 1)
...
RESULT CHECK 
drwxr-xr-x 102 k020a01 kbds0520 32768 12월 14 23:33 PRJ02.23.12.14.M1_J1.MISS_0
drwxr-xr-x 102 k020a01 kbds0520 36864 12월 15 04:52 PRJ02.23.12.14.M1_J1.MISS_1
drwxr-xr-x 102 k020a01 kbds0520 28672 12월 15 13:51 PRJ02.23.12.14.M1_J1.MISS_3
...
# cell line 빈도 확인 
ABCS = pd.read_csv('/home01/k020a01/02.Trial/M1_J1_DATA/M1_J1.MISS_2.A_B_C_S_SET.csv')
ABCS = pd.read_csv('/home01/k020a01/02.Trial/M2_J3_DATA/M2_J3.MISS_1.A_B_C_S_SET.csv')
ABCS = pd.read_csv('/home01/k020a01/02.Trial/M3_J3_DATA/M3_J3.MISS_2.A_B_C_S_SET.csv', low_memory = False)


path = '/home01/k020a01/02.Trial/M1_J1_DATA/'
path = '/home01/k020a01/02.Trial/M2_J3_DATA/'
path = '/home01/k020a01/02.Trial/M3_J3_DATA/'



plotname = 'cell_freq.M3.MISS2'
C_list = list(ABCS.DC_cellname)
