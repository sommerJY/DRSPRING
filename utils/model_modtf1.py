...
from utils_tf import EarlyStopping, after_process
...
def training_model(data, save_file, device, es_value, jobname, max_epoch,learning_rate,drop_pert,hid_dim,ghid_dim,b_size,layer_count,glayer_count,hidden_layer_size):
    ...
    for epoch in range(max_epoch):
        ...
        with torch.no_grad():
            ...
            if epoch==999:
                ...
                torch.save(model.state_dict(), path3+"./" + jobname+ "_trained_model.pt")
                result_df.to_csv(path3+"./" + jobname+ "_result_df.csv")
...
def testing_model(data, save_file, saved_model, basal, jobname, device, learning_rate,drop_pert,hid_dim,ghid_dim,b_size,layer_count,glayer_count,hidden_layer_size):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
    criterion = nn.MSELoss()
    test_df= pd.read_csv("./raw/M1_test_tf.csv")  
    gi_info_lm_list = test_df.iloc[:,5:354].columns.to_list()
    ccle_info_file =  "./raw/M1_ccle_lincs_convert.csv"
    ...
    pred_np_processed.to_csv(save_file+'/'+ jobname +'_predicted_expression.csv')
