...
################    
class FuTrainh3dtHSDB_L3Dataset(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None):
        super(FuTrainh3dtHSDB_L3Dataset, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])    #
    #
    @property
    def raw_file_names(self):
        return ['train.csv', 'lb_drug_to_smiles.csv','HSDB_network.csv','lincs_wth_ccle_org_all.csv']
    #
    @property
    def processed_file_names(self):
        return ['train_hsdb.pt']
    #
    def process(self):
        data_list = []
        maxNumAtoms = 50
        data_df = pd.read_csv(self.raw_paths[0])
        drug_df = pd.read_csv(self.raw_paths[1], names=['pert_id','smiles'])
        ...
        lincs_wth_ccle_org_all2_filt = lincs_wth_ccle_org_all2[['cell_id']+df_full_num.iloc[:,6:355].columns.to_list()]
        ...
        for i in range(0,len(df_full_num_filt)):
            ...
            exp_data = torch.FloatTensor(df_full_ccle_lst['exp']).view(349, 1) 
            ...
            data_list.append(PairData_DT(x_s=x_data, edge_index_s=edge_data, y=y_data, \
                smiles=s_data, x_p=exp_data, edge_index_p=gene_edge, dose = d_data, time = t_data, \
...
################    
class FuDevh3dtHSDB_L3Dataset(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None):
        super(FuDevh3dtHSDB_L3Dataset, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])    #
    #
    @property
    def raw_file_names(self):
        return ['dev.csv', 'lb_drug_to_smiles.csv','HSDB_network.csv','lincs_wth_ccle_org_all.csv']
    #
    @property
    def processed_file_names(self):
        return ['dev_hsdb.pt']
...
################    
class FuTesth3dtHSDB_L3Dataset(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None):
        super(FuTesth3dtHSDB_L3Dataset, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])    #
    #
    @property
    def raw_file_names(self):
        return ['test.csv', 'lb_drug_to_smiles.csv','HSDB_network.csv','lincs_wth_ccle_org_all.csv']
    #
    @property
    def processed_file_names(self):
        return ['test_hsdb.pt']
...
################    
class FuNewh3dtHSDB_L3Dataset(InMemoryDataset):
    def __init__(self, root,new_drug_cellline, new_smiles, new_basal): # 
        ...
        super(FuNewh3dtHSDB_L3Dataset, self).__init__(root, new_drug_cellline, new_smiles, new_basal) #
        self.transform = None
        self.data, self.slices = torch.load(self.processed_paths[0])    #이거 안하면 여러개가 겹챠지지가 않아
    #
    @property
    def raw_file_names(self): 
        return  [self.new_drug_cellline, self.new_smiles2, 'HSDB_network.csv', 'ccle_lincs_convert.csv', self.new_basal,'test.csv']
    #
    @property
    def processed_file_names(self):
    ...
    def process(self):
        ...
        if self.new_basal == 'lincs_wth_ccle_org_all.csv' and len(data_df[data_df.cell_id.isin(ccle_lincs_df['ccle_name'])])>0:
            ...
            data_df = data_df[['pert_id','cell_id','pert_idose','pert_itime','sig_id','pert_type']]
        drug_df = pd.read_csv(self.raw_paths[1], names=['pert_id','smiles'])
        drug_df.pert_id = drug_df.pert_id.astype(str)
        hu_gene_lm2 = pd.read_csv(self.raw_paths[2], index_col=0, dtype='str')
        ...
        lincs_wth_ccle_org_all2_filt = lincs_wth_ccle_org_all2[['cell_id']+df_full_test.iloc[:,6:355].columns.to_list()]
        df_full_num_filt = df_full_num[df_full_num.cell_id.isin(lincs_wth_ccle_org_all2_filt.cell_id)].reset_index(drop=True,inplace=False)  # 2977
        df_full_ccle = pd.merge(df_full_num_filt[['sig_id', 'pert_id', 'pert_type', 'cell_id', 'pert_idose']], lincs_wth_ccle_org_all2_filt,how='left', on = "cell_id")
        gene_edge = from_gene_to_node_HSDB_L3(df_full_ccle, hu_gene_lm2)
        for i in range(0,len(df_full_num_filt)):
            ...
            df_full_ccle_lst = pd.DataFrame(df_full_ccle.iloc[i,5:354])
            df_full_ccle_lst.columns = ["exp"]
            exp_data = torch.FloatTensor(df_full_ccle_lst['exp']).view(349, 1) #torch.Size([978, 1])
            d_data = torch.FloatTensor(np.array(df_full_num_filt['pert_idose'][i])).view(1, 1)
            t_data = torch.FloatTensor(np.array(df_full_num_filt['pert_itime'][i])).view(1, 1)
